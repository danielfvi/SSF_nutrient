...
#nut = "Iron"
  ...
  # Format data
  mpa_dta_sf <- world %>% 
    left_join(total_nut_supply %>% filter(nutrient == nut), by=c("gu_a3"="iso3c"))
  
  # Spatialize tiny
  sdata_pt <- world_centers %>% 
    left_join(total_nut_supply %>% filter(nutrient == nut), by=c("iso3"="iso3c")) %>% 
...
g
# Export
ggsave(g, filename = "Figures/Figure SX - SSF nutrient supply seafood.pdf", 
       width=10, height=6.2, units="in", dpi=600, device=cairo_pdf)

ggsave(g, filename = "Figures/Figure SX - SSF nutrient supply seafood.jpeg", 
       height = 6.2, 
       width = 10)
