...
plot1 = ggplot(data = SSF_intake, aes(y=perc_SSF, x=intake, label = iso3c, color = category)) +
  ...
  geom_text_repel(min.segment.length = 0.5, seed = 42, box.padding = 0.5, size = 5, color = "black", max.overlaps = 12) +
...
##Pacific
SSF_pacific = SSF_scores %>% 
  filter(continent %in% c("Micronesia", "Melanesia", "Polynesia"))
