...
ggplot(data = percent_nutrient_supply) +
  geom_boxplot(aes(y = perc_SSF)) +
  facet_wrap(~nutrient, scales = "free")

percent_nutrient_supply$perc_SSF[percent_nutrient_supply$nutrient=="Calcium" & percent_nutrient_supply$perc_SSF>50] = 50
#percent_nutrient_supply$perc_SSF[percent_nutrient_supply$nutrient=="DHA+EPA" & percent_nutrient_supply$perc_SSF>25] = 25
#percent_nutrient_supply$perc_SSF[percent_nutrient_supply$nutrient=="Protein" & percent_nutrient_supply$perc_SSF>3] = 3
percent_nutrient_supply$perc_SSF[percent_nutrient_supply$nutrient=="Zinc" & percent_nutrient_supply$perc_SSF>20] = 20
#percent_nutrient_supply$perc_SSF[percent_nutrient_supply$nutrient=="Vitamin B12" & percent_nutrient_supply$perc_SSF>10] = 10
percent_nutrient_supply$perc_SSF[percent_nutrient_supply$nutrient=="Iron" & percent_nutrient_supply$perc_SSF>20] = 20
percent_nutrient_supply$perc_SSF[percent_nutrient_supply$nutrient=="Vitamin A" & percent_nutrient_supply$perc_SSF>10] = 10
...
breaks_list <- list("DHA+EPA"=c(0, 50, 98),
                    "Vitamin B12"=c(0, 50, 88),
                    "Iron"=c(0, 10, 19.98),
                    "Zinc"=c(0, 10, 19.98),
                    "Calcium"=c(0, 25, 49.98),
                    "Vitamin A"=c(0, 5, 9.98))

labels_list <- list("DHA+EPA"=c("0", "50", "100"),
                    "Vitamin B12"=c("0", "50", "90"),
                    "Iron"=c("0", "10", ">20"),
                    "Zinc"=c("0", "10", ">20"),
                    "Calcium"=c("0", "25", ">50"),
                    "Vitamin A"=c("0", "5", ">10"))
...
g
# Export
ggsave(g, filename = "Figures/Figure 3 - 12_16.pdf", 
       width=10, height=6.2, units="in", dpi=600, device=cairo_pdf)

ggsave(g, filename = "Figures/Figure 3 - 12_16.jpeg", 
       height = 6.2, 
       width = 10)
...
mean(percent_nutrient_supply$perc_SSF, na.rm = T)
...
stats_nut = percent_nutrient_supply %>% 
  group_by(nutrient) %>% 
  summarise(
    max_SSF = max(perc_SSF, na.rm = T),
    min_SSF = min(perc_SSF, na.rm = T),
    perc_SSF = mean(perc_SSF, na.rm = T))
...
total_nut_supply <- read_csv("Outputs/total_nut_supply.csv") %>% 
  ...
  mutate(SSF = Artisanal + Subsistence,
         total = Artisanal + Industrial + Recreational + Subsistence + aquaculture + Import,
         perc_SSF = 100*SSF/total,
         perc_SSF_ASF = 100*SSF/total,
         nutrient = recode(nutrient, "Omega-3 fatty acids" = "DHA+EPA")) %>% 
  filter(nutrient == "DHA+EPA") %>% 
  select(iso3c, nutrient, perc_SSF, perc_SSF_ASF)
...
stats_nut = percent_nutrient_supply %>% 
  #group_by(nutrient) %>% 
  summarise(perc_SSF = mean(perc_SSF, na.rm=T),
            perc_SSF_ASF = mean(perc_SSF_ASF, na.rm=T))

stats_continent = percent_nutrient_supply %>% 
  mutate(country = countrycode(iso3c, 'iso3c', 'country.name'),
         continent = countrycode(iso3c, 'iso3c', 'region23')) %>% 
  group_by(continent, nutrient) %>% 
  summarise(perc_SSF = mean(perc_SSF))
...
percent_nutrient_supply <- percent_nutrient_supply %>% 
  #filter(!nutrient == "DHA+EPA") %>% 
  group_by(nutrient) %>% 
  summarise(perc_SSF_ASF_min = min(perc_SSF, na.rm = T),
            perc_SSF_ASF_max = max(perc_SSF, na.rm = T),
            perc_SSF_ASF = mean(perc_SSF, na.rm = T))
